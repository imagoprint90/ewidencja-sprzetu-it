...
export async function deleteLocationAction(id: string): Promise<ActionResult> {
  ...
  if (location?.is_warehouse) {
    return { ok: false, error: "Nie można usunąć domyślnej lokalizacji magazynu." };
  }
  ...
  if ((employeeCount ?? 0) > 0 || (equipmentCount ?? 0) > 0) {
    return {
      ok: false,
      error:
        "Nie można usunąć lokalizacji — jest przypisana do pracowników lub sprzętu. Zmień im lokalizację przed usunięciem.",
    };
  }

  const { error } = await supabase.from("locations").delete().eq("id", id);
  if (error) return { ok: false, error: "Nie udało się usunąć lokalizacji." };

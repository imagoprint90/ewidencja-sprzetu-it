...
export default function KategoriePage() {
  const { categories, equipment, addCategory, renameCategory, archiveCategory } = useStore();
  ...
  function handleAdd() {
    ...
    if (!result.ok) {
      setAddError(result.error ?? "Nie udało się dodać kategorii.");
      return;
    }
    setNewName("");
    setAddError(null);
  ...
  function saveEdit(id: string) {
    ...
    if (!result.ok) {
      setEditError(result.error ?? "Nie udało się zmienić nazwy.");
      return;
    }
    setEditingId(null);
  }

  function equipmentCount(categoryId: string) {
    return equipment.filter((e) => e.categoryId === categoryId).length;
  }

  function confirmArchive() {
    if (!archiveTarget) return;
    const result = archiveCategory(archiveTarget);
    if (!result.ok) {
      setArchiveError(result.error ?? "Nie udało się zarchiwizować kategorii.");
          ...
          <tbody>
            {active.map((c) => (
              <tr key={c.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3">{equipmentCount(c.id)}</td>
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-2">
                    {editingId === c.id ? (
                      <>
                        <Button size="sm" variant="secondary" onClick={() => saveEdit(c.id)}>
                          <Check size={14} />
                        </Button>
                      ...
                      <>
                        <Button size="sm" variant="secondary" onClick={() => startEdit(c.id, c.name)}>
                          <Pencil size={14} />
                          Zmień nazwę
                        </Button>

...
export default function NowySprzetPage() {
  const router = useRouter();
  const { categories, addEquipment } = useStore();
  ...
  function onSubmit(values: EquipmentFormValues) {
    ...
    if (!result.ok) {
      setSubmitError(result.error ?? "Nie udało się zapisać sprzętu.");
      return;
    }
    router.push(`/sprzet/${result.equipment!.id}`);
  }
